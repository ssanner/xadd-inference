...
Hybrid MDPs as defined above are naturally factored~\cite{boutilier99dt}
...
tabular enumerations --- because these functions can
condition on both discrete and continuous state and action variables as
...
%For now, with our hybrid MDP defined as above, we
...
The algorithm we use for solving hybrid MDPs, proposed by ~\cite{zahra}, is a continuous state and action generalization of {\it value iteration}~\cite{bellman},  which is a dynamic programming algorithm for constructing optimal policies.  It proceeds by constructing a
series of $h$-stage-to-go optimal value functions $V^h(\vec{b},\vec{x})$.
