...
$(\vec{c^*},\epsilon_{\mathit{new}}) := \argmin_{\vec{c}^*,\epsilon} \epsilon$ subject to $C$\;
\If{$\epsilon \neq \epsilon_{\mathit{new}}$}{
     $\epsilon := \epsilon_{\mathit{new}}$, go to line 4\;
}
...
%Thus to summarize our key contributions in this subsection, 
%we have managed to transform an initial bilinear saddle point
%into a bi-level linear program and then through crucial insights about
%the second level, we have managed to solve the pairwise leaf approximation
%problem efficiently with only linear programming and the technique
