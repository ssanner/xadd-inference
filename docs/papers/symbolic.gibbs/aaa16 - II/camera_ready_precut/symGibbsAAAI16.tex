...
%P_{tot} = \underbrace{M_1 V_1}_{P_1} + \underbrace{M_2 V_2}_{P_2} = 3.0
\end{align}
}%end emph
...
%In practice are closed under linear transformations. Therefore, , in theory, they can handle linear observed constraints.  However, in practice the number of pieces in the posterior can grow exponential in the number of \emph{marginalizations} i.e.\ the integrals required in equation~(\ref{e:inference}). and has altogether ignored the complications of (nonlinear) determinism.
...
%by proposing a variation of Gibbs sampling that saves a tremendous amount of redundan 
...
% with (non)linear observed constraints.  
...
%$\frac{\partial f(x_1, \ldots, x_n)}{\partial x_1}$ 
...
If $f(x_1, \cdot)$ is invertible (w.r.t.\ $x_1$),
...
The reason is that 
%in the r.h.s\ of (\ref{e:substitute}), 
...
%\emph{Hamiltonian Monte Carlo} (HMC) 

To address item (a), 
we compare the proposed \emph{closed-form Gibbs} sampler (SymGibbs) to
\emph{baseline Gibbs} (BaseGibbs) \cite{pearl1987evidential},
\emph{rejection sampling} (Rej) \cite{hammersley1964monte}, 
\emph{tuned Metropolis-Hastings} (MH)\footnote{MH is automatically tuned after \cite{roberts1997weak} by testing 200 equidistant proposal variances in interval $(0, 0.1]$ and accepting a variance for which the acceptance rate closer to 0.24.} %end footnote
% \cite{roberts1997weak} 
and  
...
%SymGibbs, BaseGibbs and MH are run on \emph{collapsed} models. 
