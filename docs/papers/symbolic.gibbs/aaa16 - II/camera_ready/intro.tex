...
To date, the applicable proposed exact inference tools are restricted to piecewise polynomial models where the partitioning boundaries are  
...
As such, in the second part of the paper we address the problem of sampling from highly piecewise densities. %by proposing a variation of Gibbs sampling that saves a tremendous amount of redundan 
We firstly introduce a rich class of \emph{piecewise fractional functions} as a building block for piecewise graphical models.% with (non)linear observed constraints.  
