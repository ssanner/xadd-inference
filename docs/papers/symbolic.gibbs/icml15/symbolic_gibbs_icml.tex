...
the state-of-the-art probabilistic programming languages (PPLs) %toolkits
...
\begin{theorem} 
\label{theorem1}
Let {\footnotesize $\pr(Z\!=\!z | x_1, \ldots, x_n) = \delta\big(f(x_1, \ldots, x_n)-z \big)$}.
...
\delta \big( f(x_1, \ldots, x_n) - z \big) dx_1 
\label{e:fand}
\end{align}
...
Gj(\cdot) = 
%\frac{\mathcal{A} \cdot Y + \mathcal{B}}{\mathcal{C} \cdot Y + \mathcal{D}}
...
, (d) and 0.00001 as well as Anglican implementation of SMC alg. with %RDB alg. with 
parameters (e)
...
Fig.~\ref{fig:mom}-c using different MCMCs.% on the {\color{green} reduced-dim} and noisy models .
...
%Due to the complexity of the posteriors that we are interested in, 
...
They use sequential importance sampling for i.i.d. data points with an observed summation.% in a minimalistic BN.
