...
that for the first time, carries out inference in models with nonlinear algebraic deterministic constraints. % that are not discussed in the literature so far.
...
In the next sections, the inference procedure and its limitations are studied in graphical models with deterministic random variables.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\incmargin{0.5em}
%\SetInd{0.5em}{1em}
\begin{algorithm}[hb!]
...
If the multivariate function $\pr(X_1, \ldots, X_N)$
has an analytic integral for all its variables $X_i$,
then 
as Algorithm~\ref{alg:analytic-cdf} shows, 
we can create a map from variables $X_i$ to their corresponding closed-form CDFs.
This map is passed to the main symbolic Gibbs sampling procedure in Algorithm~\ref{alg:symbolic-gibbs}.
During the sampling process, 
to sample $x_i \sim \pr(X_i \,|\, \bvec{x}_{-i})$
it is sufficient to get the (multivariate) closed-form CDF associated to $X_i$ from the map
and instantiate it with $\bvec{x}_{-i}$ to obtain the appropriate CDF.

By this method, instead of computing $N \times T$ integrals for $T$ samples, only $N$ integrals are computed and 
$N \times T$ function instantiations are performed which leads to a major improvement of speed.

What is remained is to show that a large subset of PPF functions indeed have closed-form integrals. 
...
and the sub-function denominators, $D_{i}$, can be factorized into polynomials ${D}_{i,h}$ where the maximum degree of each variable $x \in \bvec{x}$ in each $D_{i,h}$ is less or equal to 2 ($\text{rel-degree}(D_{i, k}) \leq 2$).%, where function $\text{rel-degree}(\cdot)$, the relative degree, is defined as follows:
 %\begin{enumerate} 
...
%\begin{align*}
%\singlecase{&(x> L_1) \wedge (x>L_2) \wedge \cdots \wedge\\
%	&(x< L_1) \wedge (x<L_2) \wedge \cdots
%(I_1>0) \wedge (I_2) \wedge \cdots}{f_1}
%\end{align*}
