...
In this paper, we introduced a general method for handling observed (non)linear algebraic constraints in graphical models via dimension reduction.
We also provided an expressive class of models (namely, PPFs) closed under dimension reduction. 
We showed that a large subset of PPFs have closed-form univariate
integrals, which together with reducing the dimensionality, enabled the
main contribution of this paper: a fully-automated exact Gibbs sampler
called \emph{closed-form Gibbs} that can sample from models with algebraic constraints.  In \emph{closed-form Gibbs}, all
univariate CDFs required for Gibbs sampling are computed analytically
and offline.  Hence, \emph{closed-form
