...
\title{Full-Automated Symbolic Gibbs Sampling in Piecewise Algebraic %Graphical 
...
MCMC sampling is an asymptotically unbiased tool that avoids the hassle of multiple integrations. % required in closed-form sampling.   
...
(e.g.\ %by \emph{adaptive rejection sampling} 
...
%Note that models with DAG dependencies between deterministic nodes can be easily converted to the canonical form (\ref{e:infer.sd}).For instance in the collision model, potential $\delta[P_\text{tot} - (P_1 + P_2)]$ can be restated as $\delta[P_\text{tot} - (M_1 V_1 + M_2 V_2)]$ which is canonical.}).
%For example, a standard form for the Collision model is: $\Phi := \pr(M_1, M_2, V_1, V_2)$ (as in equation (\ref{e:col-prior})), and $\boldsymbol{\Psi^D} := \{\Psi_{P_1}, \Psi_{P_2}, \Psi_{P_\text{tot}}\}$  where $\Psi_{P_1} := \delta[P_1 - M_1 V_1]$, $\Psi_{P_2} := \delta[P_2 - M_2 V_2]$ and $\Psi_{P_\text{tot}} := \delta[P_\text{tot} - M_1 V_1 - M_2 V_2]$.
...
\STATE {\bf Output} {unnormalized posterior joint of a subset of variables from which, all other variables can be decided.}
 \vspace{1mm}
%{
%{\bf Begin}//{
...
\begin{algorithmic}
\small
\STATE {\bf Input:} $G$ is a polynomial fraction and $X$ is a variable involved in $G$ (i.e.\ $Y \in \textsc{Scope}(G)$).
\STATE {\bf Output:} Solutions of $G$ w.r.t.\ $X$.
\STATE {\bf Let} $\mathcal{A}$ be the set of fractional polynomial functions that do not involve $X$. 
{%\small
\STATE {\bf if} $\exists A, B \in \mathcal{A}. \; (G=0) \Rightarrow (A X + B = 0)$ 
\\\hspace{5mm}
{\bf then return} $\{-B/A\}$
\STATE {\bf elseIf } $\exists A_1, B_1, A_2, B_2 \neq B_1 \in \mathcal{A}.\; (G=0) \Rightarrow$
\\\hspace{5mm} 
$ [(A_1 X + B_1)(A_2 X + B_2)]=0$
\\\hspace{5mm} 
{\bf then return} $\{-B_1/A_1, -B_2/A_2\}$ {\bf elseIf} ...
\STATE {\bf else return} $\emptyset$ \hspace*{\fill} //could not be factorized. }
\end{algorithmic}
\end{algorithm}
...
can be reconstructed.% expressed as a function of them:
...
\begin{comment}
\vspace{4pt}
\begin{mdframed}[backgroundcolor=light-gray, leftmargin = 1pt, skipabove = 0pt]
\begin{enumerate} [topsep=0pt,itemsep=-1ex,partopsep=0ex,parsep=1ex, leftmargin=1.5ex] %[nolistsep]
\item \emph{Observed variable instantiation.} 
...
%associated 
logical values $G_j(\cdot)$ (so, the potential $\delta ( G_j(\cdot) - Z_j)$ itself is replaced by 1). Note that by the definition of Dirac $\delta$, this simply means that  unobserved deterministic variables are marginalized out. \vspace{-1mm}
\begin{equation*}\footnotesize 
...
\end{equation*}
%
\item \emph{Joint factor formation.} the product of all stochastic potentials is computed: \vspace{-2.0mm}
...
\begin{comment}
\subsection{Subtleties and limitations of conditioning on determinism}
To illustrate a problem that may arise when conditioning on deterministic random variables,
firstly consider a scenario where the observation is actually softened by noise.
For instance, conditioned on $X_1 + X_2 = c$:

(a) The posterior joint of $X_1$ and $X_2$ can be approximated by 
$\pr (X_1, X_2 \,|\, Z = c)$
where $\pr(Z \,|\, X_1, X_2) := \mathcal{N}(X_1 + X_2, \sigma^2_\text{noise})$.

Nonetheless, the observation ($X_1 + X_2 = c$) can be respecified in the form: $\frac{X_1 - c}{X_2} = -1$ (for $X_2 \neq 0$)
and therefore: 

(b) The posterior can be approximated by 
{$\pr (X_1, X_2 \,|\, Z' = -1)$} where 
{$\pr(Z' \,|\, X_1, X_2) := \mathcal{N}(\frac{X_1 - c}{X_2}, \sigma'^2_\text{noise})$}.

It can be shown that regardless of the choice of noise variance parameters, 
%$\sigma^2_\text{noise}$ and $\sigma'^2_\text{noise}$,
the approximations (a) and (b) lead to different posteriors.
This issue persists in the limit where the variances tend to 0.

By definition, Dirac delta is the limit of a normal distribution.
Therefore, $\delta$-collapsing algorithm is also affected by the specification form of the determinism.
(This can simply be verified by plugging  (a) $x_1 + x_2 - c$ and (b) $\frac{x_1 - c}{x_2} + 1$ in $f(\cdot)$ in Theorem~\ref{theorem1}.)

While, in the case of \emph{noise softening}, the dependency of the posterior to the area on which noise is defined seems obvious,
the dependency of $\delta$ distributions to the specification of determinism can easily be forgotten.
To avoid the problem, determinism should always be regarded as a limit. 
That is, it should be remembered that $\delta$ indicates a distribution in the original space (even though its variance tends to zero) and therefore, affected by its neighboring regions.
Different specifications of the determinism correspond (limits of) different neighborhoods and therefore produce different results.\footnote{
This issue can be clearly linked to the Borel-Kolmogorov paradox (see e.g. \cite{kolmogorov1950foundations})
that implies that the probability of a lower dimensional set is not unambiguously defined.
}      
%Subsequently, to carry out $\delta$-collapsing, one should specify a deterministic relationship in the form of the mean of a normal noise factor that one would use if one had to approximate the determinism by noise (which is model-dependent). 

In this paper, we restrict the discussion to the case where 
the \emph{canonical} specification of the conditional can be implied by the nature of observation.
%This is that case where conditional determinism corresponds the observation of actual quantities.
For instance, in the Collision example, 
the actual quantity on which observation is taken place is the total momentum $P_\text{tot} = M_1 V_1 + M_2 V_2$.
Therefore, it is reasonable to use a symmetric noise model with mean $M_1 V_1 + M_2 V_2 - c$
when applying noise softening and for the same reason it is reasonable to specify 
the determinism as $\delta(M_1 V_1 + M_2 V_2 - c)$ when relying on $\delta$-collapsing.
 
In cases where the conditional determinism is due to an abstract relation   
not associated with a canonical specification form, 
\emph{Hausdorff measure} can be used \citep{morgan2008geometric} which leads to probabilities that are invariant w.r.t.\ their specifications 
\citep{kleibergen2004invariant}. The latter case is beyond the scope of this paper.
%neither softening by noise noise nor $\delta$-collapsing are recommended to be used directly. 
  

%{\color{red} The last subtle point in conditioning on determinism is that apparently equivalent deterministic observations may lead to different posteriors. For example evidence $(E_1 = 0)$ where $\pr(E_1)= \delta(E_1 - M V + 3)$ and evidence $(E_2 = 0)$ where $\pr(E_2) = \delta(E_2 - M + \frac{3}{V})$ lead to different posteriors. Although both may seem to formalize the same constraint $(MV = 3)$. It should be pointed out that the same issue arises when hard constraints are approximated by adding noise: $\mathcal{N}(E - MV + 3, \sigma) \neq \mathcal{N}(E -M + \frac{3}{V}, \sigma)$). The correct approximation form depends on the nature of observation. Dirac deltas can be understood as the limit of normal distributions with $\sigma \rightarrow 0$. Therefore, they inherit the same issue: The canonical way to express a deterministic relationship is a form to which one would add noise if one had to approximate the determinism which is model-dependent.}
\end{comment}
...
\item The qualities of the posteriors generated by conditioning on determinism 
(in terms of modality, continuity, shape of support, etc.).
\item The performance of Symbolic Gibbs and other MCMC methods on such posteriors
(in terms of convergence rate to the ground truth).
\item the effect of approximating hard constraints with noisy observations. 
\end{enumerate}
\vspace{-2mm}
The compared MCMC algorithms and the utilized measurements 
are introduced in 
Sections~\ref{sect:experimental.results.algorithms}
and \ref{sect:experimental.results.measures} respectively.
The experiments take place on three models presented and discussed in 
Section~\ref{sect:experimental.results.models}.
...
observation of a deterministic variable $Z$ %with prior $\delta\big( G_j(\cdot) - Z_j \big)$ 
...
Anglican syntax requires %promotes 
...
%taken place for each Markov chain $\Gamma$ (associated with each sampling algorithm running on each experimental model) 
is to compute the mean absolute error: 
\begin{equation}
\label{e:error.vs.time.measure}
{e}_{\Gamma}(t) := \frac{1}{\zeta \cdot n_\Gamma^t} 
\sum_{j=1}^{\zeta}
\sum_{i=1}^{n_\Gamma^t}
\left[\left|{q}^{(i)}_{j, \Gamma} - {q}_j^* \right|\right]
\end{equation}
...
mean and standard error of the mean absolute errors 
${e}_{1}(t)$ to ${e}_{\gamma}(t)$
is plotted. 

{\bf 2. Time to reach error threshold ($\boldsymbol\tau$).}
In experiments where the model size is variable,
mean absolute error vs time is illustrated for a couple of size configurations only
...
\subsubsection*{Ground truth $(\bvec{q}^*)$}
In low dimensional models %(such as asymmetric collision),
...
\subsubsection{Asymmetric collision model.} 
