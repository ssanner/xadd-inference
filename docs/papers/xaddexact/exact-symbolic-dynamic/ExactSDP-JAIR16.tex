...
\Begin
{
   ...
   \While{$h < H$}
   {
       ...
       {
              $Q_a^{h}(\vec{y})\,\leftarrow \,$\regress{$V^{h-1},a,\vec{y}$}\;
...
\Begin{
    $Q \leftarrow $ \remapWithPrimes{$V$} $\,$ \emph{// All $v_i \to v_i'$ ($\equiv$ all $b_i \to b_i'$ and all $ x_i \to x_i'$)} \;
...
\If {$v'$ in $R$}
	 {$Q \leftarrow R(\vec{b},\vec{b}',\vec{x},\vec{x}',a,\vec{y}) \oplus (\gamma \cdot Q)$} \;
    \ForEach { $v'$ in $Q$}  
    {
    	\If {$v'$ = $x'_j$}
    	{
    	\emph{//Continuous marginal integration}\\
         $Q \leftarrow \int Q \otimes P(x_j'|\vec{b},\vec{b}',\vec{x},\vec{x}',a,\vec{y}) \, d_{x'_j}$\;
    	}
	    \If {$v'$=$b'_i$}
    	{
    	\emph{// Discrete marginal summation}\\
         $Q \leftarrow \left[ Q \otimes P(b_i'|\vec{b},\vec{b}',\vec{x},\vec{x}',a,\vec{y}) \right]|_{b_i' = 1}$
...
\Begin{
	$LB  \leftarrow - \infty$ , 
	$UB \leftarrow + \infty$, 
	$IND,CONS \leftarrow \mathit{true}$ , 
	$\mathit{Case_{max}} \,\leftarrow\, \emptyset$ \;
     \For {$\phi_i \in f$ (\emph{For all partitions of $f$})}  
     {
     	\For{ $c_i \in \phi_i$ (\emph{For all conditions $c$ of $\phi_i$})}
     	{     		
     		\If {$c_i \leq y $} {$LB \,\leftarrow\, casemax(LB, c_i )$ \emph{//Add  $c_i$ to  LB, take max of all LBs}\; }
     		\If {$c_i \geq y $}	{$UB \,\leftarrow\, casemin(UB,c_i )$ \emph{//Add $c_i$ to  UB, take min of all UBs} \;	}
     		\Else {$IND \,\leftarrow\, [IND,c_i ]$ \emph{//Add constraint $c_i$ to  independent constraint set}\;}
     	}    	
     }
	$\mathit{Root} \leftarrow \solveForVar(y, f_i)$ \;
     	
     	\If {($\mathit{Root} \neq \mathit{null}$)}
     	 {
     	 		$CONS \leftarrow (\mathbb{I} \left[ LB \right] \leq \mathbb{I} \left[ \mathit{Root} \right] ) 
     	 		\wedge (\mathbb{I} \left[ \mathit{Root} \right] \leq \mathbb{I} \left[ UB \right] )$\;
     	 } 
     	 \Else
     	  {
     	 	$CONS \,\leftarrow \, (\mathbb{I} \left[ LB \right] \leq \mathbb{I} \left[ UB \right])$\;
     	 }
     	\emph{//Conditions and value of continuous max for this partition\; }   
     	$\mathit{Max} \,\leftarrow\,  IND \wedge CONS : \casemax (f_i\left \lbrace y/LB \right \rbrace, f_i \left \lbrace y/UB \right \rbrace 		,f_i \left \lbrace y/ \mathit{Root} \right \rbrace)$\;
     	\emph{//Take maximum of this partition and all other partitions\;}       
     	$\mathit{Case_{max}} \,\leftarrow\, max(\mathit{Case_{max}} ,\mathit{Max}) $  
...
Similar to case statements that allow arbitrary functions,  a general XADD allows arbitrary decision nodes and leaf expressions. However, in this paper, we restrict our functions to linear decisions and linear leaf expressions. This allows us to prune inconsistent linear constraints using an LP-solver, resulting, in general, in a more compact XADD. Next we provide the mathematical definition of \emph{linear XADDs}. %in the following: % Note that in the (or quadratic decisions that can be linearized). Also leaf expressions are assumed to be linear. 
...
% TODO: Checar se é preciso adicionar um algoritmo para continuous maximixation e SDP usando DDs.
...
% TODO: O próximo parágrafo deve ser modificado para explicar o exemplo do  continuous maximization do Luis
...
\Begin{
    ...
    $hight \leftarrow$ \prune{$F_\mathit{h}$, $\psi \wedge  F_\mathit{dec}$} \;
    \Return{\getNode{$F_\mathit{dec},high,low$}}\;
}
...
In case the current decision node is not on an inconsistent path, both low and high branches need to be checked for inconsistency. Line 16 adds the decision constraint $\neg F_\mathit{dec}$ to $\psi$ and call \emph{PruneInconsistent} for $F_l$. Line 17 performs the same for the high branch ($F_h$) adding $F_\mathit{dec}$. 

At this stage the current subtree with the computed low and high branches does not contain any inconsistent paths. Thus the algorithm returns a reduced node computed using \emph{GetNode} in Line 17. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\incmargin{.5em}
\linesnumbered
\begin{algorithm}[t!]
