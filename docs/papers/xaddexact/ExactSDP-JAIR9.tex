...
{\footnotesize
\begin{align}
R = \sum_{i=0}^K R_i = \sum_{i=0}^K \begin{cases}
d \wedge (x_i \geq 150) &: 150 - 0.1 * a_i - 0.05 * x_i \\
d \wedge \neg(x_i \geq 150) &:   x_i - 0.1 * a_i - 0.05 * x_i \\
\neg d \wedge (x_i \geq 50) &: 50 - 0.1 * a_i - 0.05 * x_i  \\
\neg d \wedge \neg(x_i \geq 50) &: x_i - 0.1 * a_i - 0.05 * x_i  \\
\end{cases} 
+ 
\begin{cases}
(x_i \leq 0) \wedge (\sum_{j \neq i} x_j \geq C) &: -\infty  \\	
(x'_i \leq 0) \wedge (\sum_{j \neq i} x'_j \geq C) &: -\infty  \\	
(x_i \geq 0) \wedge (\sum_{j \neq i} x_j \leq C)\\ \hspace{3mm}\wedge
(x'_i \geq 0) \wedge (\sum_{j \neq i} x'_j \leq C)&: 0
\end{cases} 
\label{rew_inv}
\end{align}}
where $C$ is the total capacity for $K$ items in the inventory. The second case checks the safe ranges of the capacity such that the inventory capacity of each item above zero and the sum of total capacity below $C$ is desired.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\vspace{5mm}
\begin{figure}[t!]
\centering
...
d \land ((150 \leq x \leq 300) \vee (0 \leq x \leq 100)) &:  300 - x \\
d \land (100 \leq x \leq 150)  &:  200 - x \\
\neg d \land (200 \leq x \leq 500) &:  0 \\
\neg d \land ((0 \leq x \leq 50)\vee (50 \leq x \leq 200)) &:  200 - x \\
\end{cases}
\end{align}
