...
of the intended Q-function. In \textsc{CAIC} discrete marginalization of the boolean state variable $d$ is not performed since there is no primed version of this variable $d'$ in the current Q-function. 
...
%Because the reward and transition functions for \textsc{Discrete Action}  \MarsRoverL\ use piecewise linear case statements, we note the optimal value function in this domain is also piecewise linear.  
%To show results of the pruning techniques of the previous section, Hence in this domain, we use a linear constraint feasibility checker to prune unreachable paths in the XADD, we also compare solutions for \MarsRover with and without this pruning.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{figure*}[t]
%\centering
