...
import hgm.sampling.MetropolisHastings;
...
public class GibbsSamplerTest {
	...
	public void testUniform() {
		...
		int uId = context.buildCanonicalXADDFromString("([U(x, " + (max + min) / 2.0 + ", " + (max - min) / 2.0 + ", "
					+ (max - min) / 2.0 + ")])");
		...
		testStatistics("x", numSamples, sampler, min, max, (min + max) / 2.0, Math.pow(max - min, 2) / 12.0, epsilon);
	}
	...
	public void testTriangular() {
		...
		double epsilon = 0.5; // tolerable divergence

		int triangularId = context
					.buildCanonicalXADDFromString("([T(x, " + c + ", " + (c - a) + ", " + (b - c) + ")])");
		XADD.XADDNode triangular = context._hmInt2Node.get(triangularId);
		fixVarLimits(context, triangular, -100d, 100d);
		Sampler sampler = new GibbsSampler(context, triangular);

		testStatistics("x", numSamples, sampler, a, b, (a + b + c) / 3.0, (a * a + b * b + c * c - a * b - a * c - b
					* c) / 18.0, epsilon);
		
		sampler = new MetropolisHastings(context, triangular);

		testStatistics("x", numSamples, sampler, a, b, (a + b + c) / 3.0, (a * a + b * b + c * c - a * b - a * c - b
					* c) / 18.0, epsilon);
		
		
		sampler = new RejectionSampler(context, triangular);
	...
	public void testUniformInTriangle() {
		...
		int nodeId = context.buildCanonicalXADDFromString(("([1*x > 0] " + "([1*x + 1*y < B] "
					+ "       ([1*x - W*y < B] ([1]) ([0])) " + "       ([0]) ) " + "([0]))").replaceAll("B", "" + b)
					.replaceAll("W", "" + w));
		...
		double c = 0;
		testStatistics("y", numSamples, sampler, a, b, (a + b + c) / 3.0, (a * a + b * b + c * c - a * b - a * c - b
					* c) / 18.0, epsilon);
	}

	private void testStatistics(String varName, int numSamples, Sampler sampler, double min, double max, double mean,
				double variance, double epsilon) {
		// actual...
		...
		System.out.println("actualStandardDev-expectedStandardDev = "
					+ Math.abs(Math.sqrt(aVariance) - Math.sqrt(variance)));
	...
	public void basicTest1D() {
		...
		sampler = new MetropolisHastings(context, root);
		t = tTest1D("x", sampler, numSamples, originMean, originVariance);
	...
	private double tTest1D(String varName, Sampler sampler, int numSamples, double expectedMean, double expectedVariance) {
		double sum = 0.0;
	...
	public void basicTest2D() {
		XADD context = new XADD();
		int id1 = context.buildCanonicalXADDFromString("([N(x, 10, 1,1)])");// ("([1.0 * x])");
		int id2 = context.buildCanonicalXADDFromString("([N(y, 5, 1,1)])");// ("([1.0 * x])");
		...
		VarAssignment mean = null;
		final double n = 1500000;
		System.out.println("samples from Gibbs");
		Sampler sampler = new GibbsSampler(context, root);
		for (int i = 0; i < n; i++) {
			VarAssignment assign = sampler.sample();
			// System.out.println("t = " + assign);
			mean = addVarAssign(mean, assign);
		}
		System.out.println("mean = " + multVarAssign(mean, 1.0 / 1));

		mean = null;
		System.out.println("samples from Metropolis-Hastings");
		sampler = new MetropolisHastings(context, root);
		for (int i = 0; i < n; i++) {
			VarAssignment assign = sampler.sample();
			// System.out.println("t = " + assign);
		...
		mean = null;
		System.out.println("samples from Rejection");
		sampler = new RejectionSampler(context, root);
		for (int i = 0; i < n; i++) {
			VarAssignment assign = sampler.sample();
			// System.out.println("t = " + assign);
			mean = addVarAssign(mean, assign);
	...
	private VarAssignment multVarAssign(VarAssignment a, final double d) {
		if (a == null) {
			return null;
		}
		for (String key : a.getContinuousVarAssign().keySet()) {
			a.getContinuousVarAssign().put(key, a.getContinuousVar(key) * d);
		}
		return a;
	}

	private VarAssignment addVarAssign(VarAssignment a, VarAssignment b) {
		if (a == null) {
			return b;
		}
		for (String key : b.getContinuousVarAssign().keySet()) {
			a.getContinuousVarAssign().put(key, a.getContinuousVar(key) + b.getContinuousVar(key));
		}
		return a;
	}
