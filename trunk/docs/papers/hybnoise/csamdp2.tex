...
The problem of when and how much to drain reservoirs so as to maximize
electricity revenue over time while avoiding reservoir overflow and underflow is a major problem in the OR literature ~\cite{Mahootchi2009,Yeh1985}.  Here we focus on the problem of deciding between \emph{drain} or \emph{not drain} for each reservoir to gain maximum reward over time. While the amount of rain effects the reservoir water levels, it is uncertain due to weather changes. 
\end{example*}
For simplicity, we work with a one-level water reservoir. The reward function is used to prevent overflow and underflow by assigning penalty to water levels outside the safe range. For the $\mathit{drain}$ action this is formally defined as:
\vspace{-2mm} 
{\footnotesize
\begin{align*}
R &= 
\begin{cases}
(200 \leq l_1 \leq 4500) \wedge (200 \leq l'_1 \leq 4500) &:1\\
\text{otherwise} &: -\infty\\
\end{cases}
\end{align*}
}
Electricity is generated during the $\mathit{drain}$ action, so a reward equal to the new water level $l'_1$ is assigned for the safe range of $\mathit{no}$-$\mathit{drain}$ action. We assume rainfall occurs on 4 subsequent days, followed by 4 dry days which is modelled by a counter of 3 boolean variables $d_i $ ($i \in \lbrace 1,2,3 \rbrace$). The piecewise dynamics and noise function are defined using the following PLEs:
{\footnotesize
\begin{align*}
...
\\
& n = \begin{cases}
(d_1) \wedge (1200 \leq n \leq 2000) &:-\infty \\
\neg (d_1) \wedge (0 \leq n \leq 400)&:-\infty \\
\text{otherwise} &: +\infty
\end{cases}
\end{align*}
}
For rainy days, legal noise bounds are on the higher water levels while on dry days it occurs on lower water levels. Here use of the $\delta[\cdot]$ function ensures that the continuous CPF over $x'$ integrates to 1.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
