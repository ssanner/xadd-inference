...
The sets of all deterministic potentials is denoted by $\boldsymbol{\Psi}^D$.
The remaining potentials $\boldsymbol{\Psi}^S := \boldsymbol{\Psi} \backslash \boldsymbol{\Psi}^D$ are called stochastic and do not involve Dirac deltas. Therefore:
...
to transform Stochastic-Deterministic GMs to pure stochastic GMs with lower dimensions
(see Algorithm~\ref{alg:collapsing-mechanical}).
Using MCMC methods, the variables of the latter GM can be sampled.
Provided with these samples, (the values of) the collapsed variables are reconstructed. 
