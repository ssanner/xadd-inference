...
Considering that $N$ univariate integrals should be computed per sample, approximate Gibbs samplers are typically slow.
...
In case factorization is impossible or some roots are \emph{multiple} (i.e.\ not distinct), $\emptyset$ is returned. 
\end{algorithmic}
\end{algorithm}
...
(required in (\ref{e:cdf})) and in general case this is not possible.
...
Plots of Figure~\ref{fig:mom} illustrate the
