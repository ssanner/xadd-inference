...
%CW If {\footnotesize$\textsc{Solve}\big( f(x_1, \ldots, x_n) - z ; \, x_1 \big)$} denotes the set of solutions of 
%CW $f(x_1, \ldots, x_n)-z$ w.r.t.\ $x_1$:
...
\begin{multline}
\label{e:theorem1}
%CW p_{\boldsymbol{X}\backslash X_1 | Z}(x_2, \ldots, x_n \,|\, Z=z) \propto \\
p(x_2, \ldots, x_n \,|\, Z=z) \propto \\
%CW \sum_{x_1^i \in \textsc{Solve}\big( f(\cdot) - z ; \, x_1 \big)} 
...
%
%CW &\int_{x_1=-\infty}^{\infty}p(x_1, \ldots, x_n)p(Z=z \,|\, x_1, \ldots, x_n) dx_1 \notag\\
&\int p(x_1, \ldots, x_n)p(Z=z \,|\, x_1, \ldots, x_n) \dd x_1 \notag\\
%
%CW =&\int_{x_1=-\infty}^{\infty}p(x_1, \ldots, x_n)
=&\int p(x_1, \ldots, x_n)
...
\sum_{x_1^i \in \mathcal{X}_1}
%CW \frac{\int_{x_1 = -\infty}^{\infty} p(x_1, x_2, \ldots, x_n)  \delta[x_1 - x_1^i] \dd x_1}
...
%\begin{equation}
%  \int_{-\infty}^{\infty} \delta(\alpha x) \dd x =  \int_{-\infty}^{\infty} \delta(x) \frac{\dd x}{|\alpha|}.
%\end{equation}
%The correct measure to use is now defined by a \emph{consistency} principle which states that marginalisation
%over the constraint variable $z$ must lead to the same density whichever form was chosen for the deterministic
%relation. In the above example:
%\begin{align}
%   \int p(x_1,\dots,x_n) \delta(E_1 - M V + 3) \dd E_1 \equiv \\\int p(x_1,\dots,x_n) \delta(E_2 - M + \frac{3}{V}) \dd E_2
%\end{align}
