...
%more here?
Little progress has been made in the recent years in developing \emph{exact} solutions for HMDPs with multiple continuous state variables beyond the subset of HMDPs  which have an optimal \emph{hyper-rectangular piecewise linear value function} \cite{feng04,li05}. Also previous work on \emph{exact} solutions to multivariate continuous state \emph{and} action settings have also been limited to the control theory literature for the case of linear-quadratic Gaussian (LQG) control \cite{lqgc}. 
...
We can also consider the more general CA-HMDP setting to this problem: 
\vspace{2mm}
...
The transition functions for state $x_i$ and $d$ and actions $a_i$ are defined as: 
{%\footnotesize
\vspace{-2mm}
\begin{align}
x'_i= \begin{cases}
...
The reward is the sum of $K$ functions as below:
{%\footnotesize
\begin{align}
R = \sum_{i=0}^K R_i = \begin{cases}
(x_i \leq 0) \wedge (\sum_{j \neq i} x_j \geq C) &: -\infty  \\	
(x'_i \leq 0) \wedge (\sum_{j \neq i} x'_j \geq C) &: -\infty  \\	
d \wedge (x_i \geq 150) \wedge (\mathit{safeLimits}) &: 150 - 0.1 * a_i - 0.05 * x_i \\
d \wedge \neg(x_i \geq 150) \wedge (\mathit{safeLimits})&:   x_i - 0.1 * a_i - 0.05 * x_i \\
\neg d \wedge (x_i \geq 50) \wedge (\mathit{safeLimits})&: 50 - 0.1 * a_i - 0.05 * x_i  \\
\neg d \wedge \neg(x_i \geq 50) \wedge (\mathit{safeLimits})&: x_i - 0.1 * a_i - 0.05 * x_i  \\
\end{cases} \label{rew_inv}
\end{align}}
where $C$ is the total capacity for $K$ items in the inventory and $\mathit{safeLimits}$ is the inventory capacity of each item above zero and the sum of total capacity below $C$ for all states:
\begin{align}
\mathit{safeLimits} = (x_i \geq 0) \wedge (\sum_{j \neq i} x_j \leq C) \wedge
(x'_i \geq 0) \wedge (\sum_{j \neq i} x'_j \leq C) \nonumber
\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\vspace{5mm}
\begin{figure}[t!]
...
%\hspace{-20mm}
\vspace{-3mm}
\includegraphics[width=1 \textwidth]{Figures2/diagrams/v2-inv4.pdf}
\vspace{-2mm}
...
($a = \pi^{*,2}(x)$) to achieve value $V^2(x)$.}
\label{fig:inv_policy}
\vspace{-5mm}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Note that illegal state values are defined using $-\infty$, in this case having the capacity lower than zero at any time and having capacity higher than that of the total $C$. If our objective is to maximize the long-term \emph{value} $V$ (i.e.,the sum of rewards received over an infinite horizon of actions), we show that the optimal value function can be derived in closed-form. For a single-item continuous state and action inventory CAIC problem the optimal value function is defined as below:
%from inventory1-8
{%\footnotesize
%\begin{tabular}{l}
%$
\begin{align}
V = \begin{cases}
\neg (\mathit{safeLimits}) &: -\infty \\
d \land (x \geq 300) \land (\mathit{safeLimits}) &:  277.5 - 0.1 * x \\
d \land (150 \leq x \leq 300) \land (\mathit{safeLimits})&:  232.5 + 0.05 * x \\
d \land (100 \leq x \leq 150) \land (\mathit{safeLimits}) &:  20.5 + 1.67 * x\\
d \land (x \leq 100) \land (\mathit{safeLimits})&:  82.5 + 1.05 * x \\
\neg d \land (x \geq 200) \land (\mathit{safeLimits}) &:  132.5 - 0.1 * x \\
\neg d \land (50 \leq x \leq 200) \land (\mathit{safeLimits})&: 102.5 + 0.05 * x \\
\neg d \land (x \leq 50) \land (\mathit{safeLimits})&:  52.5 + 1.05 * x \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
Note that for all cases except the very first case of $d \land (x \geq 150)$ we have ordering of $S-s-x$ when $x<s$ and no ordering when $x>s$. %For this case, since the upper bound is not explicitly defined $x \leq 200$ it seems to be incorrect, but considering all constraints, this falls under the $x<s$ category thus ordering $300-x$ items. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
According to this we rewrite Scarf's policy: 
\begin{align}
\pi^{*,2}(x) = 
\begin{cases}
d \land (x \geq 300) \land (\mathit{safeLimits}) &:  0 \\
d \land (150 \leq x \leq 300) \land (\mathit{safeLimits})&:  300 - x \\
d \land (100 \leq x \leq 150) \land (\mathit{safeLimits}) &:  200\\
d \land (x \leq 100) \land (\mathit{safeLimits})&:  300 - x \\
\neg d \land (x \geq 200) \land (\mathit{safeLimits}) &:  0 \\
\neg d \land (50 \leq x \leq 200) \land (\mathit{safeLimits})&: 200 - x\\
\neg d \land (x \leq 50) \land (\mathit{safeLimits})&:  200 - x \\
\end{cases}
\end{align}
...
(i) a state transition model $P(\vec{b}',\vec{x}'|\cdots,a,\vec{y})$, which specifies the
...
\begin{subfigure}
                \centering
                \includegraphics[width=0.55\textwidth]{Figures1/diagrams/probabilities.pdf}
        \end{subfigure}
        \vspace{-3mm}
\caption{\footnotesize Network topology between state variables in the 2-item continuous action \InventoryControl (CAIC) problem (Left); Dynamic bayes network (DBN) structure representing the transition and reward function (Middle); transition probabilities in terms of CPF and PLE for the state variables in the example(Right). }
\label{fig:dbn}
\vspace{-3mm}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\vec{b}$, $\vec{b}'$, and $\vec{x}$. Figure ~\ref{fig:dbn} presents the DBN for our CAIC example according to this definition.  
...
and linear inequalities over $\vec{x}$.  
...
Due to the same restrictions for CA-HMDPs we allow the reward function $R(\vec{b},\vec{x},a,\vec{y})$ to be either of the following:
...
Q_a^{h}(\vec{b},\vec{x},\vec{y}) & = 
 \sum_{\vec{b}'} \hspace{-1.0mm} \int \hspace{-1.0mm} \left( \prod_{i=1}^n P(b_i'|\vec{b},\vec{x},a,\vec{y}) \prod_{j=1}^m P(x_j'|\vec{b},\vec{b}',\vec{x},a,\vec{y}) \right) \label{eq:qfun} \\ 
& \Bigg[ R(\vec{b},\vec{x},a,\vec{y}) + \gamma \cdot V^{h-1}(\vec{b}',\vec{x}') d\vec{x}'  \hspace{-0.2mm} \Bigg] \nonumber
\end{align}}
...
%We present the general SDP framework for value iteration in DA-DCMDPs in Algorithm~\ref{alg:vi} (\texttt{VI}). Further we define  Algorithm~\ref{alg:contMax}(\texttt{Continuous Maximization}) to deal with the final maximization in (~\ref{eq:vfun}) for continuous actions.
...
Our objective is to take a DA-HMDP or CA-HMDP as defined in Section~\ref{sec:dcmdps}, apply value
...
\item {\it Continuous Integration}: As defined in line 4--5 once we have our primed value
...
marginalization $\sum_{\vec{b}'}$ in~\eqref{eq:qfun} which is shown in lines 7--9.
...
SDP applies to \emph{any} DCSA-DCMDP using bounded symbolic function 
...
We next formally define the XADD operations and algorithms required to support all case operations of SDP. Most importantly we prove minimality of the XADD representation and provide pruning algorithms based on this theorem. 
...
For BDDs and ADDs proof of canonicity can be defined according to \cite{bryant}. In these two cases for any function $f(x_1,\cdots, x_n)$ and a fixed variable ordering over $x_1,\cdots, x_n$, a reduced BDD or ADD is defined as the
minimally sized ordered decision diagram representation of a function $f$. This proves that there is a unique canonical BDD or ADD representation for
every function from $\{0,1\}^{n} \rightarrow \{0,1\}$ for BDDs and $\{0,1\}^{n}
\rightarrow \mathbb{R}$ for ADDs. 
...
First we provide some definitions used in the proof:
...
%\begin{proof}
%To prove the nontrivial direction, suppose $f,g \in k\left[ x_1,\cdots,x_n \right]$ give the same function on $k^n$. By hypothesis, the polynomial $f - g$ vanishes at all points of $k^n$ which imples that $f-g$ is the zero polynomial (all of its coefficients are zero). This is the result of a proven proposition where $f=0$ in  $k\left[ x_1,\cdots,x_n \right]$ if and only if $f$ is the zero function. This proves that $f=g$ in $k\left[ x_1,\cdots,x_n \right]$.
%\end{proof}
...
Given that there exists a minimal XADD for any function from $\mathbb{R}^{n+m} \rightarrow \mathbb{R}$, we next
describe how this reduced XADD can be constructed from an arbitrary ordered decision diagram.
All algorithms that we will deﬁne in the following sections rely on the helper function \emph{getNode} in Algorithm \ref{algGetNode} , which returns a minimal representation of a single internal decision node. 

The algorithm \emph{ReduceXADD} allows the efficient construction of a unique minimal XADD representation from an arbitrary ordered decision diagram with polynomial leaves and polynomial inequalities as the decision nodes. Algorithm~\ref{algReduceXADD} is defined according to the following definition: 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\incmargin{1em}
\linesnumbered
\begin{algorithm}[t!]
...
with linear XADDs) and then performs a satisfiability check to prune redundant paths  --- and tested these algorithms on different problems. For DA-DCMDPs we have used the standard problem of \Knapsack\ and two versions of a Mars Rover domain that we call \MarsRoverL\ and \MarsRoverNL.

For CA-DCMDPs we evaluated SVI on a didactic nonlinear
\MarsRover\ example and two problems from Operations Research (OR) \InventoryControl\ defined in the introduction and \WaterReservoir  all of which are described below.\footnote{All Java source code and a human/machine readable file format for all domains needed to reproduce
...
express the \MarsRover\ CA-DCMDP using piecewise dynamics and reward:
\vspace{-4mm}
\begin{align*} 
...
reservoir overflow and underflow.  Cast as a CA-DCMDP, we 
...
l_2'& =(400 * e + l_2 - 500 * e) \\
\end{align*}}
Given the descriptions of these domains and the \InventoryControl problem in the introduction, we now provide the empirical results obtained from implementing our algorithms.
...
the XADD --- later we will compare solutions for \MarsRoverL\ with and
...
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In Figure~\ref{fig:all_lin}, for both the \textsc{Discrete Action}  \MarsRoverL\ and
\textsc{Discrete Action}  \MarsRoverNL\ domains, we show how the number of nodes of the value function XADD (proportional to the space required to represent the
value function) varies for each iteration (horizon) and different
problem sizes (given by the number of pictures).  We first note that
the nonlinear variant appears \emph{much} harder for SDP (much more
time required and larger value functions) than for the linear variant
--- this is largely due to the fact that the XADD can be optimally
pruned in the linear variant.  Secondly, we note an apparent
superlinear growth in space and time required to solve each problem as
a function of the number of picture points --- this likely reflects
the superlinear growth of combinations of pictures that must be
jointly considered as the number of pictures increases.  Finally, from these
graphs it is hard to summarize general algorithm behavior as a function of
horizon, but it appears for the linear problem variant that both the time 
and space grow linearly as a function of horizon --- this will be
confirmed in the next experiments.  

Figure~\ref{fig:lin3} shows the amount of time for each iteration of
SDP vs. horizon for \MarsRoverL\ with three picture points for SDP
with and without XADD pruning.  Here we see an impressive reduction in
time and space as a function of horizon with pruning.  Without
pruning, both time and space grow super-linearly with the horizon,
while with pruning time and space appear to grow linearly with the
horizon.
...
In Figure~\ref{fig:plot3D}, we show the exact optimal value function
on the vertical axis 
for three domains, \Knapsack\, \textsc{Discrete Action}  \MarsRoverL ~and \textsc{Discrete Action}  \MarsRoverNL, as a function of two continuous state variables shown on the horizontal axes.
...
value function plot for the \MarsRoverNL~domain demonstrates nonlinear
...
within the radius limits from~\eqref{rew:nonlinear}.
...
The most relevant vein of related work for DA-DCMDPs is that of \cite{feng04} and \cite{li05} which can perform exact dynamic programming on
...
%of this work.  
As for CA-DCMDPs, there has been prior work in control theory. The field of linear-quadratic Gaussian (LQG) control \cite{lqgc} which use linear dynamics with continuous actions, Gaussian noise, and quadratic
