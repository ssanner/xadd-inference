...
%The core insight is that by treating each piecewise factor as a mixture distribution single-piece distributions and carrying out Gibbs sampling on such a model. This is equivalent with limiting the number of ``active" partitions in each step of sampling rather than dealing with the total space.
