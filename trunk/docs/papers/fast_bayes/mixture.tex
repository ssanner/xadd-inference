...
It represents a Bayesian inference model in which likelihood functions are piecewise.\footnote{Clearly, prior distributions can be piecewise as well but since exponential blow up in the posterior structure is due to the amount of data, our focus is on likelihood functions.}
For readability, the case statements are numbered and without loss of generality, it is assumed that the number of partitions in each likelihood function is $M$:
\begin{align}
\label{e:expanded}
