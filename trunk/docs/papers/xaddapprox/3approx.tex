...
%\begin{equation} \min_{\vec{c}} \max_{i=1,2} \max_{j=1..n_i} \max_{sig=+,-} \max_{\vec{x} \in Polyt(\theta_{ij})} \big( sig(\vec{c} - \vec{c}_i)\cdot \vec{x} \big)  \label{eq:optimglo} \end{equation}\vspace{-2mm}
