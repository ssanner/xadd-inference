...
%,  \hat{Q}^{h}(\vec{x}, a_1, a_2) 
\right)  
%\forall a_2 \in A_2 \right\} 
, \mathbb{I} \right)
\end{multline}
}%
...
piecewise constant $V^0(\vec{x})$ and that SDP operations are closed
form, the resulting $V^{h+1}(\vec{x})$ is also linear piecewise constant.
