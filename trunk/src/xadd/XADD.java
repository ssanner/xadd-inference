...
public class XADD {
	...
	public int computeMax(int xadd, CAction action, String _var) {
		XADDLeafMax max = new XADDLeafMax(action,_var);
		reduceProcessXADDLeaf(xadd, max, false);
		return max._runningMax;
	}
	
	public class XADDLeafMax extends XADDLeafOperation {
		
		int _runningMax; // XADD for the running max of all leaf substitutions
		CAction _action;
		double _VarCoef;
		...
		public int processXADDLeaf(ArrayList<Decision> decisions, ArrayList<Boolean> decision_values, ArithExpr leaf_val) 
		{
			...
			for (int p = 0; p<_action._actionParam.size(); p++)
			{
				if (_action._actionParam.get(p).equals(_actionString))
				{
					...
					lower_bound.add(new DoubleExpr(_action._contBounds.get(p)));
					upper_bound.add(new DoubleExpr(_action._contBounds.get(p+1)));
					continue;
				}
			}
