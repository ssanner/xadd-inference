  ...
  //Debug Flags 
    ...
    private void addParDec(int node, int dec, int par){
        ...
        for(HashSet<Integer> path: parentPaths){
            ...
            if (context._alOrder.get(Math.abs(dec) )instanceof ExprDec) {extendPath.add(dec);}
                          ...
                          ArrayList<HashSet<PointKey>> points)
    {
        ...
        for (int i = 0;i<linVars;i++) { upBound[i] = XADD.DEFAULT_UPPER_BOUND; loBound[i] = XADD.DEFAULT_LOWER_BOUND;}
                          ...
                          ArrayList<HashSet<PointKey>> points, double errorLimit)
    {
        ...
        int bound_i=0;
        for(;bound_i<functionVars;bound_i++) { upBound[bound_i] = XADD.DEFAULT_UPPER_BOUND; loBound[bound_i] = XADD.DEFAULT_LOWER_BOUND;}
        for (;bound_i<linVars;bound_i++) { upBound[bound_i] = XADD.DEFAULT_UPPER_BOUND; loBound[bound_i] = 0d;}
    ...
    public int pruneUnionPath(int root_id, double allowError){
        ...
        while(current != null){
            ...
            if (n instanceof XADDTNode){
                ...
                IntPair entry;
                if (low_child instanceof XADDTNode){ entry = new IntPair(context._alOrder.size(),node._low);}
                ...
                if (high_child instanceof XADDTNode){ entry = new IntPair(context._alOrder.size(),node._high);}
