...
public abstract class ExprLib {
	...
	public enum ArithOperation {
		UND("UND"), SUM("+"), MINUS("-"), PROD("*"), DIV("/"), ERROR("ERROR");
	...
	public enum CompOperation {
		UND("UND"), GT(">"), GT_EQ(">="), LT("<"), LT_EQ("<="), EQ("=="), NEQ(
				"!=");
	...
	public final static ArithExpr POS_INF = new DoubleExpr(
			Double.POSITIVE_INFINITY);
	public final static ArithExpr NEG_INF = new DoubleExpr(
	...
	public static abstract class Expr implements Comparable<Expr> {
		...
		public int compareTo(Expr o) {
			...
			if (!this_class.equals(other_class)) {
				...
				if (rank_this == null)
					rank_this = 5; // Put function expressions after main
									// arithmetic expressions (0-4)
				if (rank_other == null)
					rank_other = 5;
	...
	public static class CompExpr extends Expr {
		...
		public static CompExpr Convert2CompExpr(FOPC.PNode res) {
			...
			if (res._nPredID != FOPC.PNode.INVALID) {
				...
				case FOPC.PNode.EQUALS: {
					type = res._bIsNegated ? CompOperation.NEQ
							: CompOperation.EQ;
				}
					break;
				case FOPC.PNode.LESS: {
					type = res._bIsNegated ? CompOperation.GT_EQ
							: CompOperation.LT;
				}
					break;
				case FOPC.PNode.LESSEQ: {
					type = res._bIsNegated ? CompOperation.GT
							: CompOperation.LT_EQ;
				}
					break;
				}
		...
		// This ignores the difference between strict and non-strict
		// inequality... technically
		// requires a continuous function, or if piecewise, could have errors at
		// piece boundaries.
		public Expr makeCanonical() {
			...
			case NEQ:
				System.err
						.println("WARNING: XADDs should not use NEQ/EQ EXPR... can substitute on EQ: "
								+ new_expr);
				break;
			}

			// // TREATMENT OF DIVISION -- NOT CURRENTLY BEING USED BUT LEAVING
			// IN
			...
			// temp_expr._rhs = removeDivFromOtherTerms((OperExpr)
			// temp_expr._rhs, div);
			// else if (temp_expr._rhs instanceof OperExpr)
			// temp_expr._rhs = (OperExpr) ArithExpr.op(temp_expr._rhs, div,
			// PROD);
			...
			// temp_expr._lhs = (OperExpr) ArithExpr.op(temp_expr._lhs, div,
			// PROD);
			// temp_expr._lhs = (ArithExpr) temp_expr._lhs.makeCanonical();
			//
			// }
			...
			// CompExpr current_expr = new CompExpr(temp_expr._type, new_lhs,
			// ZERO);
		...
		// private ArithExpr checkDivisor(OperExpr changing_expr, ArithExpr
		// divisor) {
		// if (divisor == null) {
	...
	public abstract static class ArithExpr extends Expr {
		...
		public ArithExpr differentiateExpr(String diff_var) {
			...
			if (this instanceof OperExpr
					&& ((OperExpr) this)._type == ArithOperation.SUM) {
				ret_expr = ((OperExpr) this)
						.differentiateMultipleTerms(diff_var);
				...
				OperExpr temp = new OperExpr(ArithOperation.PROD,
						Arrays.asList(this));
		...
		public ArithExpr integrateExpr(String integration_var) {
			...
			if (this instanceof OperExpr
					&& ((OperExpr) this)._type == ArithOperation.SUM) {
				ret_expr = ((OperExpr) this)
						.integrateMultipleTerms(integration_var);
			} else if (this instanceof OperExpr
					&& ((OperExpr) this)._type == ArithOperation.PROD) {
		...
		// Takes ArithExpr expr1 linear in var, returns (coef,expr2) where expr1
		// = coef*x + expr2
		// Assume expression is canonical
		public CoefExprPair removeVarFromExpr(String remove_var) {
			...
			if (this instanceof OperExpr
					&& ((OperExpr) this)._type == ArithOperation.SUM) {
				ret = ((OperExpr) this)
						.removeVarFromExprMultipleTerms(remove_var);
			} else if (this instanceof OperExpr
					&& ((OperExpr) this)._type == ArithOperation.PROD) {
				...
				System.out
						.println("removeVarFromExpr: Unsupported expression '"
								+ this + "'");
				System.exit(1);
			}
			return ret;
	...
	public static class OperExpr extends ArithExpr {
		...
		public OperExpr(ArithOperation type, List<ArithExpr> terms) {
			...
			if ((_type == ArithOperation.MINUS || _type == ArithOperation.DIV)
					&& terms.size() != 2) {
		...
		public static ArithExpr Convert2OperExpr(FOPC.TFunction t) {
			...
			if (type == ArithOperation.UND)
				return null;
			if ((type == ArithOperation.SUM || type == ArithOperation.PROD)
		...
		public Double evaluateRange(HashMap<String, Double> low_assign,
			...
			boolean subterm_use_low = (_type == ArithOperation.MINUS
					|| _type == ArithOperation.DIV || (_type == ArithOperation.PROD && accum < 0d)) ? !use_low
					: use_low;
		...
		public boolean checkTermCanonical() {
			...
			if (_type == ArithOperation.PROD) {
				...
				if (_terms.get(_terms.size() - 1) instanceof DeltaFunExpr) // Delta
																			// comes
																			// last
																			// if
																			// it
		...
		public Expr makeCanonical() {
			...
			boolean contains_delta = new_terms.get(new_terms.size() - 1) instanceof DeltaFunExpr; // Delta
																									// is
																									// always
																									// last
			// if (contains_delta) System.err.println(new_type + ": " +
			// new_terms);
					...
					if ((e instanceof OperExpr)
						...
						for (ArithExpr e2 : sum_terms) {
							for (ArithExpr e3 : ((OperExpr) e)._terms) {
								...
								// " * " + e3);
								new_sum_terms.add(ArithExpr.op(e2, e3,
										ArithOperation.PROD));
							}
						}
						...
						for (int j = 0; j < sum_terms.size(); j++) {
							...
							sum_terms.set(j, new OperExpr(ArithOperation.PROD,
									e, e2));
						}
					}
				}
			...
			// Product terms of delta may not be canonical because they were
			// ignored for canonicalization earlier.
				...
				for (ArithExpr e : new_terms) {
					if ((e instanceof OperExpr && ((OperExpr) e)._type == ArithOperation.PROD)
						...
						if (e instanceof VarExpr || e instanceof FunExpr) {
							...
							else { // This should only occur when a product term
									// contains a DeltaExpr and it was not
									// simplified
								if (!(o._terms.get(o._terms.size() - 1) instanceof DeltaFunExpr)) {
									System.err
											.println("Possible non-canonical term: "
													+ o);
									new Exception().printStackTrace(System.err);
									System.exit(1);
								}
								index.add((ArithExpr) first_term); // It's not a
																	// constant
								d = new DoubleExpr(1d);
							}
			...
			if (new_type == ArithOperation.DIV
					&& new_terms.get(0) instanceof DoubleExpr
		...
		public ArithExpr normalize() {
			...
			if (t1 instanceof DoubleExpr) {
				...
				if (t2 instanceof DoubleExpr) {
					...
					ArrayList<ArithExpr> nterms = (ArrayList<ArithExpr>) ((OperExpr) t1)._terms
							.clone();
					nterms.set(0, normal);
					...
					System.out.println("not even t2 is Double: suspicious:"
							+ normConst);
			}
			...
			if (newType == ArithOperation.SUM) {
				for (int i = 1; i < _terms.size(); i++) {
					...
					if (_terms.get(i) instanceof VarExpr) {
						System.err.println("lost var in normalize: skipping"
								+ this);
					}
					if (_terms.get(i) instanceof OperExpr) {
						OperExpr op1 = ((OperExpr) _terms.get(i)); // op1 must
																	// be PROD
						ArrayList<ArithExpr> otherTerms = new ArrayList<ArithExpr>();
						otherTerms.add(new DoubleExpr(((DoubleExpr) op1._terms
								.get(0))._dConstVal / normConst));
						for (int j = 1; j < op1._terms.size(); j++)
							otherTerms.add(op1._terms.get(j));
						newTerms.add(new OperExpr(ArithOperation.PROD,
								otherTerms));
					}
				}
			} else {
				for (int j = 1; j < _terms.size(); j++)
					newTerms.add(_terms.get(j));
			}
		...
		public OperExpr differentiateMultipleTerms(String diff_var) {
			...
			for (ArithExpr e : this._terms) {
				if (e instanceof OperExpr) {
					differentiated_terms.add(((OperExpr) e)
							.differentiateTerm(diff_var));
		...
		public OperExpr integrateMultipleTerms(String integration_var) {
			...
			for (ArithExpr e : this._terms) {
				if (e instanceof OperExpr) {
					integrated_terms.add(((OperExpr) e)
							.integrateTerm(integration_var));
				} else if ((e instanceof VarExpr) || (e instanceof DoubleExpr)) {
					OperExpr temp = new OperExpr(ArithOperation.PROD,
							Arrays.asList(e));
		...
		public CoefExprPair removeVarFromExprMultipleTerms(String remove_var) {
			if (this._type != ArithOperation.SUM) {
				System.out
						.println("removeVarFromExprMultipleTerms: Expected SUM, got '"
								+ this + "'");
				System.exit(1);
			}
			double var_coef = 0d;
			...
			for (ArithExpr e : this._terms) {
				if (e instanceof OperExpr) {
					CoefExprPair p = ((OperExpr) e)
							.removeVarFromExprTerm(remove_var);
					var_coef += p._coef;
					...
					System.out
							.println("removeVarFromExprMultipleTerms: Unsupported expression '"
									+ e + "'");
					System.exit(1);
				}
			}
			return new CoefExprPair(new OperExpr(ArithOperation.SUM,
					remaining_terms), var_coef);
		}
		...
		public CoefExprPair removeVarFromExprTerm(String remove_var) {
			...
			if (this._type != ArithOperation.PROD) {
				System.out
						.println("removeVarFromExprTerm: Expected PROD, got '"
								+ this + "'");
				System.exit(1);
			}
			...
			for (ArithExpr e : this._terms) {
				if (e instanceof DoubleExpr) {
					...
					System.out
							.println("removeVarFromExprTerm: Unsupported expression '"
									+ e + "'");
					System.exit(1);
				}
			}
			...
			if (remove_var_count > 0) {
				if (remove_var_count > 1 || factors.size() > 0) {
					System.out.println("removeVarFromExprTerm: var '"
							+ remove_var
				...
				var_coef += coef;
				return new CoefExprPair(ZERO, var_coef); // Just add a zero term
															// in this place
			} else {
				factors.add(0, new DoubleExpr(coef));
				return new CoefExprPair(new OperExpr(ArithOperation.PROD,
						factors), var_coef);
			}
		}
		...
		public int determineHighestOrderOfVarMultipleTerms(String var) {
			if (this._type != ArithOperation.SUM) {
				System.out
						.println("determineHighestOrderOfVarMultipleTerms: Expected SUM, got '"
								+ this + "'");
				System.exit(1);
			}
			int max_order = 0;
			for (ArithExpr e : this._terms) {
				if (e instanceof OperExpr) {
					max_order = Math.max(max_order,
							((OperExpr) e).determineHighestOrderOfVarTerm(var));
				} else if (e instanceof VarExpr) {
					OperExpr temp = new OperExpr(ArithOperation.PROD,
							Arrays.asList(e));
					max_order = Math.max(max_order,
							temp.determineHighestOrderOfVarTerm(var));
				} else if (!(e instanceof DoubleExpr)) {
					System.out
							.println("determineHighestOrderOfVarMultipleTerms: Unsupported expression '"
									+ e + "'");
					System.exit(1);
				}
			}
		...
		public int determineHighestOrderOfVarTerm(String var) {
			if (this._type != ArithOperation.PROD) {
				System.out
						.println("determineHighestOrderOfVarTerm: Expected PROD, got '"
								+ this + "'");
				System.exit(1);
			}
			...
			for (ArithExpr e : this._terms) {
				if (e instanceof DoubleExpr) {
					...
					System.out
							.println("determineHighestOrderOfVarTerm: Unsupported expression '"
									+ e + "'");
					System.exit(1);
				}
			}
	...
	public static class DoubleExpr extends ArithExpr {
		...
		public boolean equals(Object o) {
			if (o instanceof DoubleExpr) {
				...
				if (this._dConstVal == d._dConstVal)
					return true;
				else {
					Double dif = this._dConstVal - d._dConstVal;
					if ((Double.isInfinite(dif) || Double.isNaN(dif)))
						return false;
		...
		public Expr makeCanonical() {
			if (Double.isInfinite(_dConstVal) || Double.isNaN(_dConstVal)
					|| this == ONE || this == ZERO || this == NEG_ONE)
				return this;
			return this.round();
		}
		...
		public ArithExpr round() {
			if (Double.isInfinite(_dConstVal) || Double.isNaN(_dConstVal)
				...
				return this;
			return new DoubleExpr(
					(Math.round(_dConstVal * XADD.ROUND_PRECISION) * 1d)
							/ XADD.ROUND_PRECISION);
		}
	...
	public abstract static class FunExpr extends ArithExpr {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder();
			boolean omit_parens = _args.size() == 1
					&& _args.get(0) instanceof OperExpr;
			for (ArithExpr e : _args)
				sb.append((sb.length() != 0 ? ", " : "") + e.toString());
			return _funName + (omit_parens ? "" : "(") + sb.toString()
					+ (omit_parens ? "" : ")");
		}
		...
		public final static Class ARRAYLIST_ARITH_EXPR_CLASS = new ArrayList<ArithExpr>()
				.getClass();
		...
		public ArithExpr substitute(HashMap<String, ArithExpr> subst) {
			...
			try {
				Constructor<? extends FunExpr> constructor = getClass()
						.getDeclaredConstructor(ARRAYLIST_ARITH_EXPR_CLASS);
	...
	public static class DeltaFunExpr extends FunExpr {
		...
		@Override
		// Might set to Double.POSITIVE_INFINITY, but we'll take a discrete
		// intepretation and set it to 1
		public Double evaluate(HashMap<String, Double> cont_assign) {
