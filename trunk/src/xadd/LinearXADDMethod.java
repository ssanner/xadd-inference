...
public class LinearXADDMethod{
	...
	public LinearXADDMethod(int localRoot, XADD global){
		...
		for(int k=0;k<nContextCVars; k++){ cVarID2localID[k]=-1;} //Flag invalid Vars
		int i=0;
            ...
            if (o._type == ArithOperation.PROD) {
                if (o._terms.size() != 2)
                    ...
                    if (index < 0) {
                        ...
                        for(int k=0;k< nLocalCVars;k++){ System.err.print( globalContinuousVarList.get(localID2cVarID[k]) + " ");}
                    }
