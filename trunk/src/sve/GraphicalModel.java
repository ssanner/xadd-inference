...
import xadd.XADDParseEvent;

public class GraphicalModel implements Observer {
	...
	public class Factor {
		...
		public XADD _localContext; // Outside classes cannot see _context, so
									// store locally
	...
	public GraphicalModel(String filename) {
		...
		XADD.addParseEvent(this);
		ArrayList l = HierarchicalParser.ParseFile(filename);
		// System.out.println(l);
		// XADDParseEvent pe = new XADDParseEvent(this);
		parseGMTemplate(l);
	...
	public void instantiateGMTemplate(HashMap<String, ArrayList<Integer>> var2expansion) {
		...
		// with a separate x_1... both x_i' and x_1 (or lowest index) must be
		// present
			...
			for (String var : (ArrayList<String>) list) {
				...
				if (split.length == 1 || (split.length == 2 && Character.isDigit(split[1].charAt(0)))) {
					...
					for (int i = 0; i < index_values.size(); i++) {
						...
						if (prime && i == 0) {
							// If using prime, get first index CPF directly from
							// cpt lookup
							...
							// Need to process all other variables in XADD and
							// substitute
	...
	protected int getXADD(String var, boolean is_bool) {
		...
		if (is_bool) {
			...
			int low_branch = _context.apply(_context.getTermNode(XADD.ONE), high_branch, XADD.MINUS);
	...
	public void parseGMTemplate(ArrayList l) {
		...
		for (int index = 0; index < _alCVarsTemplate.size(); index++) {
			...
			if (!val.trim().equalsIgnoreCase("x"))
				try {
