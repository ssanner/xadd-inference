...
We implemented the CSA-DP algorithm using the XADDs and tested it on several domains. The first example which has been used throughout the paper is the \MarsRoverNL, problems from the OR literature; \InventoryControl and \WaterReservoir. In the following subsections we will study these examples empirically.
...
Without the pruning, even in low horizons, because of infeasible branches, time and space grows exponentially. We have also used the general pruning method of XADDs as in ~\cite{sanner_uai11} for the other domain problems.
